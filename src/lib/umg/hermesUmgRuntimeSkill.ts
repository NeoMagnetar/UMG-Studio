...
export function getHermesUmgRuntimeSkillPack(): HermesUmgRuntimeSkillPack {
  return {
    ...
    instructions: [
      ...
      'Capability behavior: capabilities are not assumed tools. Resolve through the supplied toolCapabilityRegistry. Unknown/unavailable capabilities must be blocked/unavailable, not silently treated as successful external tool execution. Safe configured customer_message_draft/report_generate capabilities may create app-local non-destructive artifacts only; customer_message_draft returns draft text and never sends email.',
