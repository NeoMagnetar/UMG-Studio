...
export default function App() {
  const importedBlocks = normalizedBlocks.length ? normalizedBlocks : rawBlocks;
  const [library] = useState<UMGBlock[]>(() => normalizeImportedBlocks(importedBlocks as unknown[]));
  ...
  return <div className="app">
    ...
    <main>
      ...
      <section className="library card"><LibraryAudit report={migrationReport as any} /><ShelfControls shelves={shelves} activeShelf={activeShelf} setActiveShelf={(shelf: ShelfMode) => { setActiveShelf(shelf); setTagFilters([]); }} search={search} setSearch={setSearch} tagFilters={tagFilters} setTagFilters={setTagFilters} visibleTags={visibleTags} filtered={Boolean(search || tagFilters.length || roleFilter !== 'all' || statusFilter !== 'all')} shown={visibleItems.length} total={currentShelf.items.length} clear={() => { setSearch(''); setTagFilters([]); setRoleFilter('all'); setStatusFilter('all'); }} />{activeShelf === 'molt_blocks' && <div className="filterBar"><button className={roleFilter === 'all' ? 'hot' : ''} onClick={() => setRoleFilter('all')}>All MOLT + Meta ({libraryWithStatus.length})</button>{roles.map((role) => <button key={role} className={roleFilter === role ? 'hot' : ''} onClick={() => setRoleFilter(role)}>{labelDisplayType(role)} ({libraryWithStatus.filter((block) => block.displayType === role).length})</button>)}<button className={roleFilter === 'meta' ? 'hot metaHot' : ''} onClick={() => setRoleFilter('meta')}>Meta ({libraryWithStatus.filter((block) => block.displayType === 'meta').length})</button></div>}{activeShelf === 'molt_blocks' && <div className="filterBar small">{statuses.map((s) => <button key={s} className={statusFilter === s ? 'hot' : ''} onClick={() => setStatusFilter(s)}>{s} {String(s === 'all' ? libraryWithStatus.length : statusCounts[s] ?? 0)}</button>)}</div>}{activeShelf === 'source_audit' ? <SourceAuditTable items={visibleItems} onInspect={(item) => setInspected(item.asset)} /> : <AssetCards items={visibleItems} onAdd={addAsset} onInspect={(item) => setInspected(item.asset)} />}{activeShelf === 'molt_blocks' && <details><summary>Legacy MOLT sections / Meta visible</summary>{sections.map((section) => <div key={section.type}>{section.label} ({section.blocks.length})</div>)}</details>}</section>
...
function AssetCards({ items, onAdd, onInspect }: { items: ShelfAsset[]; onAdd: (item: ShelfAsset) => void; onInspect: (item: ShelfAsset) => void }) {
  return <div className="cards">{items.map((item, index) => <div key={`${item.id}:${item.sourcePath ?? 'local'}:${index}`} className={`block asset-${item.kind} ${item.displayType === 'meta' ? 'metaCard' : ''}`}><div className="cardtop"><b>{item.title}</b><span className="badge">{item.displayType === 'meta' ? 'Meta / non-compiler' : item.kind}</span></div><p>{item.containedRoles.join(', ') || item.status || item.kind}</p><small>{item.tags.slice(0, 8).join(', ') || 'no tags'}</small><small>{item.sourcePath ?? 'local asset'}</small><div className="row"><button onClick={() => onAdd(item)}>{item.kind === 'sleeve' ? 'Open Sleeve' : 'Add to Workspace'}</button><button onClick={() => onInspect(item)}>Inspect JSON</button></div></div>)}</div>;
}
