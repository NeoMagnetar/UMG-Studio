...
import { classifyLibraryDisplay, getLibraryAssetStatus, normalizeImportedBlocks, sectionLibraryByDisplayType } from './lib/umg/migrateLibrary';
...
const statuses: Array<'all' | LibraryAssetStatus> = ['all', 'runnable', 'warning-bearing', 'reference-only', 'unsupported', 'meta'];
...
export default function App() {
  ...
  const libraryWithStatus = useMemo(() => library.map((b) => {
    const classification = classifyLibraryDisplay(b);
    return { ...b, displayType: classification.displayType, presentationStatus: classification.status };
  }), [library]);
  ...
  const visible = libraryWithStatus.filter((b) => (filter === 'all' || b.displayType === filter) && (statusFilter === 'all' || b.presentationStatus === statusFilter) && (`${b.title} ${b.tags.join(' ')} ${b.content} ${b.legacy?.sourcePath ?? ''}`.toLowerCase().includes(search.toLowerCase())));
  const librarySections = useMemo(() => sectionLibraryByDisplayType(visible), [visible]);
  ...
  const addBlock = (b: UMGBlock) => {
    ...
    const classification = classifyLibraryDisplay(b);
    if (!classification.compilerActive && classification.displayType !== 'meta') { setStatus(`${b.title} is ${classification.status} and was not added`); return; }
    const ws = structuredClone(workspace);
    ws.sleeves[0].stacks[0].neoblocks[0].blocks.push({ ...b, id: `${b.id}_copy_${Date.now()}`, displayType: classification.displayType, presentationStatus: classification.status, defaultState: classification.displayType === 'meta' ? 'off' : b.defaultState, visibility: classification.displayType === 'meta' ? 'audit_only' : b.visibility, source: { origin: 'workspace', sourceId: b.id, version: '0.1' } });
    ws.graph = buildGraphFromSleeve(ws.sleeves[0]);
    setWorkspace(ws);
  ...
  return <div className="app"><header><div><b>UMG Studio v0.1</b><span>local graph cognition studio</span></div><nav>{['simple', 'builder', 'creator'].map((m) => <button key={m} className={mode === m ? 'hot' : ''} onClick={() => setMode(m as any)}>{m}</button>)}</nav><button onClick={() => downloadJson('umg-workspace.json', workspace)}>Export Workspace</button></header>
    <main className={`${layout.leftCollapsed ? 'leftCollapsed' : ''} ${layout.rightCollapsed ? 'rightCollapsed' : ''} ${layout.bottomCollapsed ? 'bottomCollapsed' : ''}`} style={{ ['--leftWidth' as any]: `${layout.leftWidth}px`, ['--rightWidth' as any]: `${layout.rightWidth}px`, ['--bottomHeight' as any]: `${layout.bottomHeight}px` }}>
      ...
      <section className="library card"><div className="panelTitle"><h2>Library</h2><button onClick={() => setLayout({ ...layout, leftCollapsed: !layout.leftCollapsed })}>{layout.leftCollapsed ? 'Expand' : 'Collapse'}</button></div><div className="report"><b>Migration Report</b><span>blocks {(migrationReport as any).importedBlocks}</span><span>sleeves {(migrationReport as any).importedSleeves}</span><span>HUMAN skipped {(migrationReport as any).skippedHumanReferences}</span><span>unsupported {((migrationReport as any).unsupportedRoles || []).join(', ') || 'none'}</span></div><input placeholder="search title/tags/content/source" value={search} onChange={e => setSearch(e.target.value)} /><div className="filterBar"><button className={filter === 'all' ? 'hot' : ''} onClick={() => setFilter('all')}>All ({libraryWithStatus.length})</button>{roles.map((r) => <button key={r} className={filter === r ? 'hot' : ''} onClick={() => setFilter(r)}>{labelDisplayType(r)} ({libraryWithStatus.filter((b) => b.displayType === r).length})</button>)}<button className={filter === 'meta' ? 'hot' : ''} onClick={() => setFilter('meta')}>Meta ({libraryWithStatus.filter((b) => b.displayType === 'meta').length})</button></div><select value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)}>{statuses.map((s) => <option key={s}>{s}{s !== 'all' ? ` (${statusCounts[s] ?? 0})` : ''}</option>)}</select><div className="librarySections">{librarySections.filter((section) => filter === 'all' || section.type === filter).map((section) => <section className={`typeSection ${section.type}`} key={section.type}><h3>{section.label} <span className="badge">{section.blocks.length}</span></h3><div className="cards">{section.blocks.map((b, i) => { const classification = classifyLibraryDisplay(b); return <div className={`block ${classification.status} display-${classification.displayType}`} key={`${b.id}:${b.legacy?.sourcePath ?? 'local'}:${i}`}><div className="cardtop"><b>{b.title}</b><span className="badge roleBadge">{classification.displayType === 'meta' ? 'Meta' : labelDisplayType(b.role)}</span><span className="badge">{classification.badge}</span></div><small>type {b.type} • role {b.role} • {b.category}</small><p>{b.content || 'Reference asset: no runnable content extracted.'}</p><em>{b.tags.join(', ') || 'no tags'}</em><small>source {b.legacy?.sourcePath ?? 'local sample'}</small><div className="row"><button onClick={() => { setInspectedBlock(b); setSelected(undefined); }}>Inspect JSON</button><button onClick={() => addBlock(b)}>{classification.displayType === 'meta' ? 'Add Meta Note' : 'Add workspace copy'}</button></div></div>; })}</div></section>)}</div></section>
...
function MoltWorkbench({ neoblock, selectedBlockId, library, roleTab, search, editingId, draftTitle, draftContent, output, onRoleTab, onSearch, onAddRole, onToggle, onSaveLibrary, onAddLibrary, onEdit, onDraftTitle, onDraftContent, onSaveEdit, onCancelEdit, onGenerate }: any) {
  const metaBlocks = (neoblock?.blocks ?? []).filter((b: UMGBlock) => classifyLibraryDisplay(b).displayType === 'meta');
  const filtered = library.filter((b: UMGBlock) => {
    const classification = classifyLibraryDisplay(b);
    return (roleTab === 'all' || classification.displayType === roleTab) && `${b.title} ${b.content} ${b.tags.join(' ')}`.toLowerCase().includes(search.toLowerCase());
  }).slice(0, 16);
  const renderCard = (b: UMGBlock, role: string) => <div key={b.id} className={`moltCard ${b.defaultState === 'off' ? 'off' : 'active'} ${b.id === selectedBlockId ? 'picked' : ''} ${classifyLibraryDisplay(b).displayType === 'meta' ? 'metaCard' : ''}`} style={{ ['--roleColor' as any]: role === 'meta' ? '#94a3b8' : MOLT_ROLE_COLORS[role as keyof typeof MOLT_ROLE_COLORS] }}><div className="cardtop"><b>{b.title}</b><span className="badge">{classifyLibraryDisplay(b).displayType === 'meta' ? 'Meta / non-compiler' : b.defaultState}</span></div>{editingId === b.id ? <><input value={draftTitle} onChange={(e) => onDraftTitle(e.target.value)} /><textarea value={draftContent} onChange={(e) => onDraftContent(e.target.value)} /><div className="row"><button onClick={() => onSaveEdit(b.id)}>Save</button><button onClick={onCancelEdit}>Cancel</button></div></> : <><p>{b.content || 'Empty block content.'}</p><small>{b.tags.join(', ') || 'no tags'}</small><div className="row"><button onClick={() => onToggle(b.id)}>Toggle on/off</button><button onClick={() => onEdit(b)}>Edit</button><button onClick={() => onSaveLibrary(b)}>Save to Library</button></div></>}</div>;
  return <div className="moltWorkbench"><div className="workbenchHeader"><div><h3>MOLT Block Workbench</h3><p>Active stack summary: {neoblock?.title ?? 'No NeoBlock selected'} • {neoblock?.blocks?.filter((b: UMGBlock) => b.defaultState !== 'off' && classifyLibraryDisplay(b).compilerActive).length ?? 0} active compiler / {neoblock?.blocks?.length ?? 0} total</p></div><div className="row"><button onClick={onGenerate}>Generate / Regenerate</button><button onClick={() => navigator.clipboard?.writeText(output || neoblock?.blocks?.filter((b: UMGBlock) => classifyLibraryDisplay(b).compilerActive).map((b: UMGBlock) => b.content).join('\n\n') || '')}>Copy Output</button></div></div><div className="roleMenu">{MOLT_ROLE_ORDER.map((role) => <button key={role} style={{ borderColor: MOLT_ROLE_COLORS[role] }} onClick={() => onAddRole(role)}>+ {labelDisplayType(role)}</button>)}<button className="metaAdd" onClick={() => onAddLibrary({ id: `meta_${Date.now()}`, title: 'Meta Note', type: 'molt_block', role: 'instruction', displayType: 'meta', presentationStatus: 'meta', content: 'Reference-only note.', tags: ['meta'], defaultState: 'off', visibility: 'audit_only', source: { origin: 'generated', version: '0.1' }, legacy: { original: { kind: 'meta-note' }, migrationWarnings: ['meta / non-compiler asset'] } })}>+ Add Meta Note</button></div><div className="moltGrid">{MOLT_ROLE_ORDER.map((role) => <section key={role} className="roleColumn" style={{ borderColor: MOLT_ROLE_COLORS[role] }}><h4 style={{ color: MOLT_ROLE_COLORS[role] }}>{labelDisplayType(role)}</h4>{(neoblock?.blocks ?? []).filter((b: UMGBlock) => b.role === role && classifyLibraryDisplay(b).displayType !== 'meta').map((b: UMGBlock) => renderCard(b, role))}</section>)}{metaBlocks.length > 0 && <section className="roleColumn metaColumn"><h4>Meta</h4>{metaBlocks.map((b: UMGBlock) => renderCard(b, 'meta'))}</section>}</div><aside className="moltLibrary"><div className="row"><input placeholder="search library" value={search} onChange={(e) => onSearch(e.target.value)} />{(['all', ...MOLT_ROLE_ORDER] as const).map((role) => <button key={role} className={roleTab === role ? 'hot' : ''} onClick={() => onRoleTab(role)}>{labelDisplayType(role)}</button>)}</div>{filtered.map((b: UMGBlock) => { const classification = classifyLibraryDisplay(b); return <div className="block mini" key={b.id}><b>{b.title}</b><small>{classification.displayType} • {classification.badge}</small><button onClick={() => onAddLibrary(b)}>{classification.displayType === 'meta' ? 'Add Meta Note' : 'Add workspace copy'}</button></div>; })}</aside><div className="outputPanel"><h4>Output Panel</h4><p>{output || 'Compile/generate output will appear here.'}</p></div></div>;
}
...
function labelDisplayType(type: string) { return type === 'all' ? 'All' : type === 'meta' ? 'Meta' : type.charAt(0).toUpperCase() + type.slice(1); }
