...
function BuilderShelfHeader({ roleFilter, shown, total, aiInstructionCount }: { roleFilter: string; shown: number; total: number; aiInstructionCount: number }) {
  return <div className="shelfHero builderHero"><b>Active builder shelf: MOLT Blocks</b><span>Active role subsection: {labelDisplayType(roleFilter)}</span><span>{shown} of {total} individual MOLT cards visible</span><span>AI instruction cards available: {aiInstructionCount}</span></div>;
}
...
function AssetCards({ items, onAdd, onInspect }: { items: ShelfAsset[]; onAdd: (item: ShelfAsset) => void; onInspect: (item: ShelfAsset) => void }) {
  return <div className="cards builderCards">{items.map((item, index) => {
    ...
    return <div key={`${item.id}:${item.sourcePath ?? 'local'}:${index}`} className={`block builderBlock asset-${item.kind} ${item.displayType === 'meta' ? 'metaCard' : ''}`}><div className="cardtop"><b>{item.title}</b><span className="badge">{item.displayType === 'meta' ? 'Meta / non-compiler' : item.kind === 'molt_block' ? 'MOLT Block Card' : item.kind}</span></div>{instId && <p className="instId">INST id: {instId}</p>}<p>role: {item.containedRoles.map(labelDisplayType).join(', ') || item.kind}</p>{category && <p>category: {category}</p>}<p>status: {item.status || 'runnable'}</p><small>tags: {item.tags.slice(0, 12).join(', ') || 'no tags'}</small><small>sourcePath: {item.sourcePath ?? 'local asset'}</small><div className="row"><button onClick={() => onAdd(item)}>{item.kind === 'sleeve' ? 'Open Sleeve' : 'Add to Workspace'}</button><button onClick={() => onInspect(item)}>Inspect JSON / Legacy Source</button></div></div>;
  })}</div>;
}
